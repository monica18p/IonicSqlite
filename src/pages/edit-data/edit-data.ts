...
export class EditDataPage {
  data = {rowid:0, date:"", type:"", description:"", amount:0, extra:10}
    ...
    .then((db: SQLiteObject) => {
      ...
      .then(res => {
        if(res.rows.length > 0) {
          ...
          this.data.extra = res.rows.item(0).extra
        }
      })
      .catch(e => {
        console.log(e)
  ...
  updateData() {
    ...
    .then((db: SQLiteObject) => {
      db.executeSql('UPDATE expense SET date=?,type=?,description=?,amount=?,extra=? WHERE rowid=?', 
      [this.data.date, this.data.type, this.data.description, this.data.amount, this.data.rowid, this.data.extra])
      .then(res => {
        console.log(res)
